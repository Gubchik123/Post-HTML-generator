...
export function get_click_event_on_create_post_btn(e) {
	if (
		...
		hidden_post_body_field.value +=
			content_block.previousElementSibling.querySelector(
				".element-content"
			).innerHTML;

		for (const element_content of content_block.querySelectorAll(
			".element-content"
		)) {
		...
		hidden_post_body_field.style.display = "block";
